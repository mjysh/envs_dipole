...
class DipoleSingleEnv(gym.Env):
    ...
    def __firstorderdt(self,t,pos,delta_gammma_normalized):
        ...
        if (self.mode == 'CFD'):
            uVK,vVK,oVK = self.__VKflow((pos[0],pos[1]))
        elif (self.mode == 'reduced'):
    ...
    def __VKflow(self, pos):
        ...
        # t1 = timer.time()
        uVK,vVK,oVK =  cf.adapt_time_interp(self.UUU,self.VVV,self.OOO,self.XMIN,self.XMAX,self.YMIN,self.YMAX,self.cfd_framerate,time = self.time,posX = posx,posY = posy)
        # t2 = timer.time()
        # print('time',t2-t1)
        uVK = -uVK
        oVK = 0
...
#        print(Style.RESET_ALL)
    ...
    def _get_obs(self):                 # get the orientation (reltative to the targeted direction) and the shape
        if (self.mode == 'CFD'):
            uVK,vVK,oVK = self.__VKflow(self.pos[0:2])
        elif (self.mode == 'reduced'):
            uVK,vVK,oVK = self.__VKflow_reduced(self.pos[0:2])
        ort = self.pos[-1]
