...
class DipoleSingleEnv(gym.Env):
    ...
    def __init__(self, mode = 'CFD', dt = 0.1, alpha = 3, r = 3,mu = 0.8, flexibility = 0.5, train_offset=0.1,cfdpath='/home/yusheng/CFDadapt/'):
        # size and parameters
        self.speed = mu
        self.bl = 1
        self.bw = 0.2
        ...
        # self.cfdinterval = cfd_readinterval
        ...
        high = np.array([np.finfo(np.float32).max,
                         ...
                         np.finfo(np.float32).max, 
                         np.finfo(np.float32).max])
        low = np.array([-np.finfo(np.float32).max,
                        -np.finfo(np.float32).max,
                        -np.finfo(np.float32).max,
                        -np.finfo(np.float32).max,
                        -np.finfo(np.float32).max, 
    ...
    def step(self, action):
        ...
        if self.pos[0]>self.permittedR or self.pos[0]<self.permittedL or self.pos[1]<self.permittedD or self.pos[1]>self.permittedU:
            terminal = True
        # reward = -abs(self._get_obs()+action*np.pi)[0]
    ...
    def __VKflow(self, pos,t):
        ...
        # uVK,vVK = cf.interp_vel_from_matrix(self.Uf,self.Vf,self.xf,self.yf,self.Of,posX=posx,posY=posy,time=t,frame_rate = self.cfd_framerate,read_interval = 5)
        t1 = timer.time()
...
#        print(Style.RESET_ALL)
    ...
    def _get_obs(self):                 # get the orientation (reltative to the targeted direction) and the shape
        ...
        if (self.mode == 'CFD'):
            uVKL,vVKL,oVKL = self.__VKflow(posLeft,self.time)
            uVKR,vVKR,oVKR = self.__VKflow(posRight,self.time)
            # uVKL,vVKL,oVKL = self.__VKflow(posFront,self.time)
            # uVKR,vVKR,oVKR = self.__VKflow(posBack,self.time)
        elif (self.mode == 'reduced'):
            uVKL,vVKL,oVKL = self.__VKflow_reduced(posLeft)
            uVKR,vVKR,oVKR = self.__VKflow_reduced(posRight)
            # uVKL,vVKL,oVKL = self.__VKflow_reduced(posFront)
            # uVKR,vVKR,oVKR = self.__VKflow_reduced(posBack)
        ort = self.pos[-1]
