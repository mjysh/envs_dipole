...
def generalization_test(args):
    ...
    ############## Hyperparameters ##############
    if args.policy_num:
        policy_paths = [args.policy_path+str(k) for k in range(args.policy_num_start,args.policy_num+1)]
    else:
...
def generalization_test_naive(args):
    from tqdm import tqdm
    if args.test_bound:
        boundL, boundR, boundD, boundU = args.test_bound
    else:
    ...
    for sr,at,ast in zip(success_rate,average_time,average_success_time):
        ...
        policy_index += 1
    from datetime import date
    with open(args.policy_path+'_generalization_naive_'+str(date.today())+'.txt','w') as resultFile:
        resultFile.writelines(res_strs)
...
def test(args):
    ...
    max_timesteps = int(args.max_timesteps)    # max timesteps in one episode
    render = False           # render the environment
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_mode', type=str ,choices=['policy_grading', 'generalization', 'single','naive_grading','sourceseeking_generalization', 'naive_single','generalization_naive'], required=True)
    ...
    parser.add_argument('--policy_num_start', type=int, default = 1)
    ...
    elif args.test_mode == 'generalization_naive':
        generalization_test_naive(args)
    elif args.test_mode == 'single':
        test(args)
    elif args.test_mode == 'naive_single':
        test_naive_policy(args)
